...
<x-guest-dashboard-layout>
    <div class="w-full text-neutral-900 font-sans antialiased" 
         x-data="{ 
            ...
            // Pop-up Notifikasi & Invoice (Clean UI Terintegrasi)
            openNotificationModal: false,
            notificationTitle: '',
            notificationMessage: '',
            isNotificationSuccess: true,
            
            // State Tambahan untuk Menampung Data Manifes Invoice Kamar
            showInvoice: false,
            invoiceData: { order_id: '', date: '', status: '', total: 0, items: [] },

            // Eksekusi trigger notifikasi kustom
            ...
            // Fungsi simulasi NFC Smart Lock
            triggerSmartLock() {
            ...
            // Fungsi AJAX untuk mengambil rincian manifest kamar lunas
        ...
        @if($currentBooking)
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 pb-4 border-b border-neutral-200">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 bg-white border border-neutral-200 p-4 w-full lg:w-auto shadow-sm">
                    <div class="text-xs">
                        ...
                        <p class="font-bold text-neutral-800 mt-0.5">{{ date('d M Y', strtotime($currentBooking->check_in_date)) }}</p>
                    </div>
                    <div class="text-xs border-l border-neutral-200 pl-4">
                        ...
                        <p class="font-bold text-neutral-800 mt-0.5">{{ date('d M Y', strtotime($currentBooking->check_out_date)) }}</p>
                    </div>
                    <div class="text-xs border-l border-neutral-200 pl-4">
                    ...
                    <div class="absolute inset-0 p-6 flex flex-col justify-between bg-gradient-to-t from-black/80 via-black/20 to-transparent">
                        ...
                        <div class="flex items-center justify-between border-t border-white/20 pt-4">
                            <div class="text-xs text-neutral-400">
                                Check-in Window: <span class="text-white font-bold">{{ date('d M Y', strtotime($currentBooking->check_in_date)) }}, 03:00 PM</span>
                            </div>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <span class="inline-flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-wider text-emerald-400">
                                <span class="h-1.5 w-1.5 rounded-full bg-emerald-400 {{ $currentBooking->room_number ? 'animate-pulse' : '' }}"></span> {{ $currentBooking->room_number ? 'Active' : 'Pending' }}
                            </span>
                        </div>
                    ...
                    <div class="flex justify-center items-center py-2">
                        <div class="bg-white p-2 border-2 border-amber-600 shadow-md">
                            <div class="w-20 h-20 bg-neutral-900 flex flex-col items-center justify-center text-white text-[9px] font-mono tracking-tighter text-center p-1">
                                ...
                                {{ $currentBooking->room_number ? 'NFC READY' : 'NO UNIT' }}
                            </div>
                        </div>
                    </div>

                    <button type="button" 
                            {{ !$currentBooking->room_number ? 'disabled' : '' }}
                            @click="triggerSmartLock()"
                            class="w-full bg-amber-700 hover:bg-amber-800 text-white font-bold text-xs uppercase tracking-widest py-3 transition-colors shadow-md disabled:bg-neutral-800 disabled:text-neutral-500 disabled:cursor-not-allowed cursor-pointer">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
                ...
                <div class="bg-white border border-neutral-200 p-6 shadow-sm flex flex-col justify-between">
                    <div>
                        ...
                        <div class="grid grid-cols-3 gap-2 text-center">
                            <button type="button" @click="serviceType = 'Housekeeping'; openServiceModal = true" class="border border-neutral-100 bg-neutral-50/50 p-3 hover:border-amber-600 transition-colors flex flex-col items-center justify-center gap-1.5 cursor-pointer">
                            ...
                            <button type="button" @click="serviceType = 'Laundry Valet'; openServiceModal = true" class="border border-neutral-100 bg-neutral-50/50 p-3 hover:border-amber-600 transition-colors flex flex-col items-center justify-center gap-1.5 cursor-pointer">
                            ...
                            <button type="button" @click="serviceType = 'Extra Towels & Pillows'; openServiceModal = true" class="border border-neutral-100 bg-neutral-50/50 p-3 hover:border-amber-600 transition-colors flex flex-col items-center justify-center gap-1.5 cursor-pointer">
                ...
                <div class="bg-white border border-neutral-200 p-6 shadow-sm flex flex-col justify-between">
                    ...
                    @php
                        // Cek status akomodasi saat ini langsung lewat database relasional internal
                        $checkStatus = DB::table('bookings')->where('id', $currentBooking->booking_id)->value('status');
                    @endphp
                    ...
                    @else
                        <button type="button" id="pay-button" onclick="triggerMidtransPayment({{ $currentBooking->booking_id }})" class="w-full bg-neutral-900 hover:bg-neutral-800 text-white font-bold text-xs uppercase tracking-widest py-3 transition-colors mt-4 cursor-pointer shadow-sm flex items-center justify-center gap-2">
                            <i class="fa-solid fa-credit-card text-[11px]"></i> View Bill & Pay
                        </button>
                    @endif
                </div>
            ...
            <div x-show="showInvoice" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-xs" x-transition x-cloak>
                <div class="relative bg-white max-w-sm w-full border border-neutral-200 p-6 shadow-2xl transform transition-all text-left" @click.away="showInvoice = false">
                    <div class="border-b border-neutral-100 pb-3 flex justify-between items-center">
                        ...
                        <button type="button" @click="showInvoice = false" class="text-neutral-400 hover:text-neutral-900 text-xs cursor-pointer"><i class="fa-solid fa-xmark"></i></button>
                    </div>
                    
                    <div class="mt-4 space-y-3">
                        <div class="text-[11px] font-medium text-neutral-500 space-y-2">
                            <p>Booking Reference ID: <span class="font-mono font-bold text-neutral-800" x-text="'#BK-' + String(invoiceData.order_id).padStart(4, '0')"></span></p>
                            ...
                            <p class="flex items-center gap-2">Billing Matrix: 
                                <span class="font-bold uppercase text-[9px] px-2 py-0.5 border text-emerald-700 bg-emerald-50 border-emerald-200">
                                    Lunas / Confirmed
                                </span>
                            </p>
                        </div>
                                ...
                                <template x-for="line in invoiceData.items">
                                    <div class="flex justify-between items-center text-[11px]">
                                        <div class="min-w-0 flex-1">
                                            ...
                                            <p class="text-[10px] text-neutral-400 font-mono" x-text="line.qty + ' Night(s) x Rp ' + new Intl.NumberFormat('id-ID').format(line.price)"></p>
                                        </div>
                        ...
                        <div class="border-t border-neutral-100 pt-3 flex justify-between items-baseline">
                            <span class="text-[10px] font-bold uppercase tracking-wider text-neutral-900">Total Paid Invoiced</span>
    ...
    <script>
        // SINKRONISASI GERBANG PEMBAYARAN MIDTRANS DENGAN MODAL POPUP JAVASCRIPT
        function triggerMidtransPayment(bookingId) {
            const payButton = document.getElementById('pay-button');
            
            // Aman & Kompatibel 100% dengan Alpine v3
