<x-guest-dashboard-layout>
        ...
        <div class="bg-white border border-neutral-200/80 p-5 flex items-center space-x-4 shadow-sm">
            ...
            <div>
                ...
                <h3 class="text-xs font-sans font-bold text-neutral-900 mt-1">
                    ...
                    @else
                        <span class="text-neutral-400 font-normal italic">No Active Itinerary</span>
                    @endif
                </h3>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div class="lg:col-span-2 bg-white border border-neutral-200 shadow-sm rounded-none overflow-hidden">
            <div class="px-6 py-4 border-b border-neutral-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex items-center space-x-2 text-[10px] font-bold uppercase tracking-wider text-neutral-500">
                    <button onclick="filterTable('all')" id="tab-all" class="filter-tab px-3 py-1.5 bg-neutral-950 text-white rounded-none">All Bookings</button>
                    <button onclick="filterTable('confirmed')" id="tab-confirmed" class="filter-tab px-3 py-1.5 hover:text-neutral-900 transition-colors">Upcoming</button>
                    <button onclick="filterTable('completed')" id="tab-completed" class="filter-tab px-3 py-1.5 hover:text-neutral-900 transition-colors">Completed</button>
            ...
            <div class="w-full overflow-x-auto custom-scrollbar">
                <table class="w-full text-left border-collapse" id="bookings-main-table">
                    <thead>
                        <tr class="bg-neutral-50 border-b border-neutral-200 text-[9px] font-bold uppercase tracking-widest text-neutral-400">
                            ...
                            <td class="py-5 px-6 text-right font-sans">
                                <div class="flex items-center justify-end space-x-1">
                                    @if($booking->status === 'pending')
                                        <button type="button" onclick="triggerMidtransCheckout({{ $booking->id }})" class="bg-amber-700 hover:bg-amber-800 text-white font-bold text-[9px] uppercase tracking-widest px-3 py-1.5 border border-transparent rounded-none transition-colors cursor-pointer">
                                            Pay Now
                                        </button>
                                    @else
                                       <button type="button" onclick="printInvoice({{ $booking->id }}, '{{ $booking->type_name }}', '{{ $booking->room_number ?? 'TBA' }}', '{{ date('d M Y', strtotime($booking->check_in_date)) }}', '{{ date('d M Y', strtotime($booking->check_out_date)) }}', {{ $booking->total_price }})" class="border border-neutral-200 hover:border-neutral-900 text-neutral-800 font-bold text-[9px] uppercase tracking-widest px-3 py-1.5 transition-colors bg-white cursor-pointer">
    Receipt
</button>
                                    @endif
                                </div>
            ...
            <div class="bg-white border border-neutral-200 p-6 shadow-sm space-y-4">
                ...
                <div class="space-y-2.5 text-xs font-medium text-neutral-500">
                    ...
                    <div class="flex justify-between items-baseline border-t border-neutral-100 pt-3 text-neutral-900">
                        ...
                        <span class="text-base font-bold font-mono text-amber-900">Rp {{ number_format($bookings->sum('total_price'), 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    // PERBAIKAN SELESAI: Logika tabulasi menyaring parameter status secara presisi
    function filterTable(status) {
        ...
        rows.forEach(row => {
            ...
            // Tab "Upcoming" menampung data 'confirmed' yang baru saja dibayar dan 'pending' yang menunggu dana
            ...
            // Tab "Completed" hanya menampung riwayat data 'completed' asli (Tamu sudah Check-Out)
   ...
   // INSTANT PRINT RECEIPT / INVOICE ENGINE (DENGAN ALOKASI NOMOR KAMAR)
        ...
        const invoiceContent = `
            ...
            <head>
                ...
                <style>
                    ...
                    .header { text-align: center; border-b: 1px solid #e7e5e4; padding-bottom: 20px; margin-bottom: 30px; }
