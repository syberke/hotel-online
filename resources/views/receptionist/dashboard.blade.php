<x-receptionist-dashboard-layout>
    ...
    <div class="space-y-6">
        <section class="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:flex-row lg:items-center lg:justify-between">
            <div><p class="text-sm font-medium text-blue-600">Front office overview</p><h2 class="mt-1 text-2xl font-semibold tracking-tight text-slate-900">Reception dashboard</h2><p class="mt-2 text-sm text-slate-500">Manage today’s arrivals, check-ins, check-outs, payments, room assignments, and room readiness.</p></div>
            <div class="flex flex-wrap gap-2"><a href="{{ route('receptionist.checkin') }}" class="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white hover:bg-blue-700"><i class="fa-solid fa-right-to-bracket text-xs"></i>Check-in</a><a href="{{ route('receptionist.roomassignment') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50"><i class="fa-solid fa-door-open text-xs"></i>Room assignment</a></div>
        </section>
            ...
            <div class="min-w-0 space-y-6">
                <article class="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <header class="flex flex-col gap-4 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <form action="{{ url()->current() }}" method="GET" class="relative w-full sm:w-80"><i class="fa-solid fa-magnifying-glass pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-xs text-slate-400"></i><input type="search" name="search" value="{{ request('search') }}" placeholder="Search guest, booking, or room" class="w-full rounded-xl border-slate-200 py-2.5 pl-11 pr-4 text-sm"></form>
                    </header>
                    ...
                    <div class="mt-4 space-y-3">
                        @forelse($attentionAlerts as $alert)
                            ...
                            <article class="rounded-xl border p-4 {{ $box }}">
                                ...
                                <a href="{{ $alert['url'] }}" class="mt-3 inline-flex items-center gap-2 text-xs font-semibold {{ $titleTone }}">{{ $alert['action'] }}<i class="fa-solid fa-arrow-right text-[10px]"></i></a>
                            </article>
                        @empty
                            <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-5 text-center text-sm font-medium text-emerald-700"><i class="fa-solid fa-circle-check mr-2"></i>No urgent operational alerts.</div>
                        @endforelse
                    </div>
                </article>
            </aside>
