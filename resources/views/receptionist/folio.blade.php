...
<x-dynamic-component :component="$folioLayout">
    ...
    <div class="space-y-5">
        <section class="flex min-w-0 flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
            <div class="min-w-0"><p class="text-sm font-semibold text-blue-600">Guest finance record</p><h2 class="mt-1 text-2xl font-semibold tracking-tight text-slate-900">Guest folio</h2><p class="mt-2 text-sm text-slate-500">Charges and payments are loaded from booking, restaurant, facility, and payment records.</p></div>
            @if($selectedBooking)
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route($backRoute) }}" class="no-print inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700"><i class="fa-solid fa-arrow-left text-blue-600"></i>Back</a>
                    <button type="button" onclick="window.print()" class="no-print inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700"><i class="fa-solid fa-print text-blue-600"></i>Print folio</button>
                    @if($staffRole === 'receptionist')
                        <a href="{{ route('receptionist.payments', ['booking_id' => $selectedBooking->id]) }}" class="no-print inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white"><i class="fa-solid fa-wallet"></i>Open payment</a>
                    @endif
                </div>
            @endif
        </section>
            ...
            <div id="folio-print-area" class="grid min-w-0 grid-cols-1 gap-5 2xl:grid-cols-[minmax(0,1fr)_340px] 2xl:items-start">
                <section class="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <header class="flex min-w-0 flex-col gap-4 border-b border-slate-100 p-5 sm:flex-row sm:items-start sm:justify-between">
                        <div class="min-w-0"><x-brand-logo class="oasis-logo-transparent h-9 w-auto" /><p class="mt-4 text-xs font-medium text-slate-400">#OA-{{ str_pad((string) $selectedBooking->id, 5, '0', STR_PAD_LEFT) }}</p><h3 class="mt-1 break-words text-lg font-semibold text-slate-900">{{ $selectedBooking->guest_name }}</h3><p class="mt-1 text-sm text-slate-500">Room {{ $selectedBooking->room_number ?? 'Unassigned' }} · {{ $selectedBooking->room_type ?? 'Room' }}</p></div>
                        <div class="min-w-0 text-sm text-slate-500 sm:text-right"><p>{{ \Carbon\Carbon::parse($selectedBooking->check_in)->format('d M Y') }} to {{ \Carbon\Carbon::parse($selectedBooking->check_out)->format('d M Y') }}</p><p class="mt-1 break-all">{{ $selectedBooking->guest_email }}</p><p class="mt-1">{{ $selectedBooking->guest_phone ?: 'No phone logged' }}</p><span class="mt-3 inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">{{ ucwords(str_replace('_', ' ', $selectedBooking->status)) }}</span></div>
                    </header>
                    ...
                    <div class="max-w-full overflow-x-auto">
                        <table class="min-w-[780px] text-left text-sm">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                ...
                                @empty
                                    <tr><td colspan="7" class="px-5 py-12 text-center text-sm text-slate-500">No charge or payment records are available for this booking.</td></tr>
                                @endforelse
                            </tbody>
