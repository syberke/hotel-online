<section id="restaurant-menu-inline-panel" class="space-y-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm scroll-mt-24">
    @php($menuCategories = ['Appetizers', 'Main Courses', 'Seafood', 'Steak Selection', 'Desserts', 'Beverages'])

    <div class="flex min-w-0 flex-col gap-5 border-b border-slate-100 pb-5 xl:flex-row xl:items-start xl:justify-between">
        <div class="min-w-0">
            <p class="text-sm font-semibold text-blue-600">Restaurant master data</p>
            <h3 class="mt-1 text-xl font-semibold tracking-tight text-slate-900">Today’s menu</h3>
            <p class="mt-2 text-sm text-slate-500">Menu items, categories, prices, images, and public availability are stored in the database.</p>
        </div>

        @if(auth()->user()->role === 'admin')
            <form action="{{ route('admin.restaurant.menu.store') }}" method="POST" class="grid min-w-0 w-full gap-3 rounded-2xl bg-slate-50 p-4 sm:grid-cols-2 xl:max-w-5xl xl:grid-cols-6">
                @csrf
                <input type="text" name="name" required placeholder="Menu name" class="min-w-0 px-3 py-2.5 text-sm xl:col-span-2">
                <select name="category" required class="min-w-0 px-3 py-2.5 text-sm"><option value="">Category</option>@foreach($menuCategories as $category)<option value="{{ $category }}">{{ $category }}</option>@endforeach</select>
                <input type="number" name="price" min="0" step="0.01" required placeholder="Price" class="min-w-0 px-3 py-2.5 text-sm">
                <input type="url" name="foto_url" placeholder="Image URL" class="min-w-0 px-3 py-2.5 text-sm sm:col-span-2">
                <textarea name="description" rows="2" placeholder="Description" class="min-w-0 px-3 py-2.5 text-sm sm:col-span-2 xl:col-span-4"></textarea>
    ...
    <div class="max-w-full overflow-x-auto">
        <table class="min-w-[1040px] text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                        ...
                        <td class="px-4 py-4 align-top">
                            <div class="flex min-w-64 items-start gap-3">
                                ...
                                <div class="min-w-0 flex-1">
                                    @if(auth()->user()->role === 'admin')
                                        <input form="{{ $updateFormId }}" type="text" name="name" value="{{ $menu->name }}" required class="w-full min-w-0 px-3 py-2 text-sm font-semibold">
                                        <input form="{{ $updateFormId }}" type="url" name="foto_url" value="{{ $menu->foto_url }}" class="mt-2 w-full min-w-0 px-3 py-2 text-xs" placeholder="Image URL">
                                    @else
                ...
                @empty
                    <tr><td colspan="6" class="px-4 py-12 text-center text-sm text-slate-500">No menu items are stored yet. Run the menu seeder or add one above.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>

    <div class="rounded-xl bg-slate-50 px-4 py-3 text-xs text-slate-500">Total menu items: <strong class="text-slate-900">{{ $menus->count() }}</strong>. Public pages only show items marked available.</div>
</section>
