        ...
        <section class="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <header class="flex min-w-0 flex-col gap-4 border-b border-slate-100 p-5 lg:flex-row lg:items-center lg:justify-between">
                ...
                <form action="{{ url()->current() }}" method="GET" class="flex w-full min-w-0 max-w-xl flex-col gap-2 sm:flex-row">
                    ...
                    <div class="relative min-w-0 flex-1"><i class="fa-solid fa-magnifying-glass pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-sm text-slate-400"></i><input type="search" name="search" value="{{ request('search') }}" placeholder="Search reservation, guest name, or email" class="w-full py-2.5 pl-11 pr-4 text-sm"></div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($todayReservations as $reservation)
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-5 py-4 text-right"><div class="flex justify-end gap-2"><a href="{{ route($folioRoute, ['booking_id' => $reservation->id]) }}" class="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">View folio</a>@if(!$isManager && $reservation->status === 'pending')<a href="{{ route('admin.finance', ['search' => $reservation->id]) }}" class="rounded-lg bg-blue-600 px-3 py-2 text-xs font-semibold text-white">Open finance</a>@endif</div></td>
                            </tr>
                        @empty
